...
namespace Xtreme
{
	internal partial class frmPayBankDetails
...
InitializeComponent();
 ...
 public  void frmPayBankDetails_old()
			//: base()
		{
		...
		public bool mIsCalledFromCOA = false;
		...
		//'added by burhan. Date:22-apr-2008

		public bool IsCalledFromCOA
		{
			get
			{
				return mIsCalledFromCOA;
			}
			set
			{
				mIsCalledFromCOA = value;
			}
		}
		...
		private void Form_Closed(Object eventSender, EventArgs eventArgs)
		{
			...
			try
			{
				if (IsCalledFromCOA)
				{
					if (frmGLChartOfAccount.DefInstance.Visible)
					{
						if (ReflectionHelper.GetPrimitiveValue<string>(SearchValue) != "")
						{
							frmGLChartOfAccount.DefInstance.FillGrid();
							frmGLChartOfAccount.DefInstance.SetDefaultRowInGrid(ReflectionHelper.GetPrimitiveValue<string>(SearchValue), 1, "G", 7);
						}
					}
				}
				...
				frmPayBankDetails.DefInstance = null;
			}
			catch (Exception exc)
			{
				NotUpgradedHelper.NotifyNotUpgradedElement("Resume in On-Error-Resume-Next Block");
		...
		public void txtParentGroupNo_Leave(Object eventSender, EventArgs eventArgs)
		{
			...
			try
			{
				...
				if (!SystemProcedure2.IsItEmpty(txtParentGroupNo.Text, SystemVariables.DataType.NumberType))
				{
					...
					mysql = mysql + " tg.l_group_name, tg.a_group_name from gl_accnt_group pg ";
					mysql = mysql + " left outer join gl_accnt_group tg on pg.m_group_cd = tg.group_cd ";
		...
		private void ShowHideMasterDetails()
		{
			...
			Frame1[mIndexGroupChildDetails].Visible = ShowSetting;
		...
		public void txtTypeCode_Leave(Object eventSender, EventArgs eventArgs)
		{
			object mTempValue = null;
			string mysql = "";

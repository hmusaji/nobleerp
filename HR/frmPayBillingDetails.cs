...
namespace Xtreme
{
	internal partial class frmPayBillingDetails
		: System.Windows.Forms.Form
	{

		
		public frmPayBillingDetails()
{
InitializeComponent();
} 
 public  void frmPayBillingDetails_old()
			//: base()
		{
		...
		private void Form_Load()
		{
			try
			{
				...
				SystemGrid.DefineSystemVoucherGridColumn(grdVoucherDetails, "Percentage", 1000, true, ColorTranslator.ToOle(Color.White).ToString(), (0x1818B4).ToString(), C1.Win.C1TrueDBGrid.AlignHorzEnum.Far, true, "", C1.Win.C1TrueDBGrid.AlignHorzEnum.Far, false, true, true, "", false, true, false, false, false, true);
				...
				SystemGrid.DefineSystemVoucherGridColumn(grdVoucherDetails, "Is Calculate", 800, true, ColorTranslator.ToOle(Color.White).ToString(), ColorTranslator.ToOle(Color.Black).ToString(), C1.Win.C1TrueDBGrid.AlignHorzEnum.Center, false, "", C1.Win.C1TrueDBGrid.AlignHorzEnum.Near, false, true, true, "", false, false);
		...
		private void cmbMastersList_RowChange()
		{
			if (grdVoucherDetails.Col == grdBillingCodeColumn || grdVoucherDetails.Col == grdBillingNameColumn)
			{
				if (grdVoucherDetails.Col == grdBillingCodeColumn)
				{
					...
					grdVoucherDetails.Columns[grdIsCalculate].Value = cmbMastersList.Columns[3].Value;
				}
				else
				{
		...
		private void DefineVoucherArray(int pMaximumRows, bool pClearArray = false)
		{
			...
			aVoucherDetails.RedimXArray(new int[]{pMaximumRows, 10}, new int[]{0, 0});
		}
		...
		public bool saveRecord(bool pApprove = false)
		{
			...
			try
			{
				...
				//'''Allowance and deduction can be added only for the first month of the payroll
				//'''for an employee
				...
				{
					MessageBox.Show("Cannot update Fixed Earning and Deduction for old employee, Please use salary variation transaction.", AssemblyHelper.GetTitle(System.Reflection.Assembly.GetExecutingAssembly()), MessageBoxButtons.OK, MessageBoxIcon.Information);
					return false;
				}
				...
				mysql = " select pbt.bill_cd from pay_employee_billing_details pebt ";
				mysql = mysql + " inner join pay_billing_type pbt on pebt.bill_cd = pbt.bill_cd ";
				...
				do 
				{
					if (aVoucherDetails.GetLength(0) == 0)
					{
						...
						try
						{
							...
							mysql = mysql + " from pay_employee_billing_details pebt ";
							mysql = mysql + " inner join pay_billing_type pbt on pebt.bill_cd = pbt.bill_cd ";
							mysql = mysql + " where pbt.show = 1 and pbt.linked_leave_cd is null ";
							mysql = mysql + " and emp_cd=" + ReflectionHelper.GetPrimitiveValue<string>(mSearchValue);
				...
				for (cnt = 0; cnt <= tempForEndVar2; cnt++)
				{
					...
					//UPGRADE_WARNING: (6021) Casting 'Variant' to Enum may cause different behaviour. More Information: https://docs.mobilize.net/vbuc/ewis#6021
					if (((TriState) Convert.ToInt32(aVoucherDetails.GetValue(cnt, grdIncludeInLeave))) == TriState.True)
					{
						mIncludeInLeave = 1;
					}
					else
					...
					//UPGRADE_WARNING: (6021) Casting 'Variant' to Enum may cause different behaviour. More Information: https://docs.mobilize.net/vbuc/ewis#6021
					if (((TriState) Convert.ToInt32(aVoucherDetails.GetValue(cnt, grdIncludeInIndemnity))) == TriState.True)
					{
					...
					mysql = " select bill_cd from pay_employee_billing_details ";
					...
					if (Convert.IsDBNull(mReturnValue))
					{
						mysql = " insert into pay_employee_billing_details ";
						mysql = mysql + " (emp_cd, bill_cd, amount, last_change_date ";
						mysql = mysql + " , comments , Percentage";
						mysql = mysql + " , include_in_leave, include_in_indemnity, user_cd) ";
						mysql = mysql + " values(";
						...
						mysql = mysql + " ," + ((SystemProcedure2.IsItEmpty(aVoucherDetails.GetValue(cnt, grdPercentage))) ? "0" : Convert.ToString(aVoucherDetails.GetValue(cnt, grdPercentage)));
					...
					{
						mysql = " update pay_employee_billing_details set ";
						mysql = mysql + " amount = " + ((SystemProcedure2.IsItEmpty(aVoucherDetails.GetValue(cnt, grdCurrentAmtColumn))) ? "0" : Convert.ToString(aVoucherDetails.GetValue(cnt, grdCurrentAmtColumn)));
						mysql = mysql + " , Percentage = " + ((SystemProcedure2.IsItEmpty(aVoucherDetails.GetValue(cnt, grdPercentage))) ? "0" : Convert.ToString(aVoucherDetails.GetValue(cnt, grdPercentage)));
						...
						mysql = mysql + " , include_in_leave =" + Conversion.Str(mIncludeInLeave);
						mysql = mysql + " , include_in_indemnity =" + Conversion.Str(mIncludeInIndemnity);
						mysql = mysql + " where emp_cd =" + ReflectionHelper.GetPrimitiveValue<string>(mSearchValue);
				...
				//UPGRADE_WARNING: (1068) GetMasterCode() of type Variant is being forced to Scalar. More Information: https://docs.mobilize.net/vbuc/ewis#1068
				mReturnValue = ReflectionHelper.GetPrimitiveValue(SystemProcedure2.GetMasterCode(" select emp_no from pay_employee where emp_cd=" + ReflectionHelper.GetPrimitiveValue<string>(mSearchValue)));
				//UPGRADE_WARNING: (1049) Use of Null/IsNull() detected. More Information: https://docs.mobilize.net/vbuc/ewis#1049
				if (!Convert.IsDBNull(mReturnValue))
				{
					//UPGRADE_WARNING: (1068) mReturnValue of type Variant is being forced to string. More Information: https://docs.mobilize.net/vbuc/ewis#1068
					mEmpNo = ReflectionHelper.GetPrimitiveValue<string>(mReturnValue);
		...
		public bool CheckDataValidity()
		{
			...
			for (cnt = 0; cnt <= tempForEndVar; cnt++)
			{
				...
				if (Convert.ToDouble(aVoucherDetails.GetValue(cnt, grdIsCalculate)) == -1)
				{
					if (!SystemProcedure2.IsItEmpty(aVoucherDetails.GetValue(cnt, grdPercentage)))
					{
						if (Convert.ToDouble(aVoucherDetails.GetValue(cnt, grdPercentage)) < 0)
						{
							MessageBox.Show("Percentage should be greater than Zero!!", AssemblyHelper.GetTitle(System.Reflection.Assembly.GetExecutingAssembly()));
							grdVoucherDetails.Col = grdPercentage;
							grdVoucherDetails.Bookmark = cnt;
							grdVoucherDetails.Focus();
							return false;
						}
					...
					{
						MessageBox.Show("Please Correct the Percentage!!", AssemblyHelper.GetTitle(System.Reflection.Assembly.GetExecutingAssembly()));
						grdVoucherDetails.Col = grdPercentage;
						grdVoucherDetails.Bookmark = cnt;
						grdVoucherDetails.Focus();
						return false;
		...
		public void GetRecord(object pSearchValue)
		{
			...
			mysql = mysql + " ,pbt.is_calculate , pebd.* ";
			mysql = mysql + " from pay_employee_billing_details pebd ";
			mysql = mysql + " inner join pay_billing_type pbt on pebd.bill_cd = pbt.bill_cd ";
			mysql = mysql + " where pebd.emp_cd = " + Conversion.Str(pSearchValue);
			...
			if (rsLocalRecDidRead)
			{
				do 
				{
					...
					aVoucherDetails.SetValue(rsLocalRec["Percentage"], cnt, grdPercentage);
					aVoucherDetails.SetValue(rsLocalRec["include_in_leave"], cnt, grdIncludeInLeave);
					aVoucherDetails.SetValue(rsLocalRec["include_in_indemnity"], cnt, grdIncludeInIndemnity);
					aVoucherDetails.SetValue(Convert.ToString(rsLocalRec["comments"]) + "", cnt, grdRemarksColumn);
					aVoucherDetails.SetValue(rsLocalRec["is_calculate"], cnt, grdIsCalculate);
					cnt++;
				}
		...
		private void grdVoucherDetails_BeforeColEdit(object eventSender, C1.Win.C1TrueDBGrid.BeforeColEditEventArgs eventArgs)
		{
			...
			{
				//UPGRADE_WARNING: (1049) Use of Null/IsNull() detected. More Information: https://docs.mobilize.net/vbuc/ewis#1049
				if ((ColIndex == grdIncludeInLeave || ColIndex == grdIncludeInIndemnity) && !Convert.IsDBNull(grdVoucherDetails.Bookmark))
				{

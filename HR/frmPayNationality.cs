...
namespace Xtreme
{
	internal partial class frmPayNationality
		: System.Windows.Forms.Form
	{

		public frmPayNationality()
{
InitializeComponent();
} 
 public  void frmPayNationality_old()
			//: base()
		{
		...
		private void frmPayNationality_Activated(System.Object eventSender, System.EventArgs eventArgs)
		{
		...
		private void Form_Load()
		{
			...
			try
			{
				...
				//assign a next code
				txtNatNo.Text = SystemProcedure2.GetNewNumber("Pay_Nationality", "Nat_no");
			}
		...
		public bool saveRecord()
		{
			...
			try
			{
				...
				{
					mysql = " INSERT INTO Pay_Nationality(user_cd, Nat_no, l_Nat_name, a_Nat_name, comments)";
					mysql = mysql + " VALUES(" + Conversion.Str(SystemVariables.gLoggedInUserCode) + ",";
					mysql = mysql + txtNatNo.Text + ",";
					mysql = mysql + "'" + txtLNatName.Text + "',";
					mysql = mysql + "N'" + txtANatName.Text + "',";
				...
				{
					mysql = " select time_stamp,protected from Pay_Nationality where Nat_cd =" + ReflectionHelper.GetPrimitiveValue<string>(mSearchValue);
					...
					//Date : 03-Jul-2007
					//Desc: Protected record can be modified but can not be deleted.
					...
					//        Exit Function
					//    End If
					mysql = "UPDATE Pay_Nationality SET ";
					mysql = mysql + " Nat_No =" + txtNatNo.Text + ",";
					mysql = mysql + " L_Nat_Name ='" + txtLNatName.Text + "',";
					mysql = mysql + " A_Nat_Name =N'" + txtANatName.Text + "',";
					mysql = mysql + " Comments =N'" + txtComment.Text + "',";
					mysql = mysql + " User_Cd =" + Conversion.Str(SystemVariables.gLoggedInUserCode);
					mysql = mysql + " where Nat_cd=" + Conversion.Str(mSearchValue);
		...
		public bool deleteRecord()
		{
			...
			bool result = false;
			string mysql = " select protected from Pay_Nationality where Nat_cd=" + ReflectionHelper.GetPrimitiveValue<string>(mSearchValue);
			//UPGRADE_WARNING: (1068) GetMasterCode() of type Variant is being forced to Scalar. More Information: https://docs.mobilize.net/vbuc/ewis#1068
			...
			if (!Convert.IsDBNull(mReturnValue))
			{
				...
				try
				{
					mysql = "delete from Pay_Nationality where Nat_cd=" + Conversion.Str(mSearchValue);
					SqlCommand TempCommand = null;
					TempCommand = SystemVariables.gConn.CreateCommand();
					TempCommand.CommandText = mysql;
					TempCommand.ExecuteNonQuery();
					...
					{
						MessageBox.Show("Nationality cannot be deleted, transaction already occurs", AssemblyHelper.GetTitle(System.Reflection.Assembly.GetExecutingAssembly()), MessageBoxButtons.OK, MessageBoxIcon.Information);
						//UPGRADE_ISSUE: (2064) ADODB.Connection method gConn.RollBackTrans was not upgraded. More Information: https://docs.mobilize.net/vbuc/ewis#2064
						SystemVariables.gConn.RollbackTrans();
						return false;
					}
		...
		public void GetRecord(object SearchValue)
		{
			...
			try
			{
				...
				mysql = " select * from Pay_Nationality where Nat_cd= " + ReflectionHelper.GetPrimitiveValue<string>(SearchValue);

				SqlCommand sqlCommand = new SqlCommand(mysql, SystemVariables.gConn);
				localRec = sqlCommand.ExecuteReader();
				if (localRec.Read())
				{
					mSearchValue = localRec["Nat_cd"];
					mTimeStamp = Convert.ToString(localRec["time_stamp"]);
					txtNatNo.Text = Convert.ToString(localRec["Nat_no"]);
					txtLNatName.Text = Convert.ToString(localRec["l_Nat_Name"]);
					txtANatName.Text = Convert.ToString(localRec["a_Nat_Name"]) + "";
		...
		public void findRecord()
		{
			...
			object mTempSearchValue = ReflectionHelper.GetPrimitiveValue(SystemProcedure.FindItem(7004000));
			//UPGRADE_WARNING: (1049) Use of Null/IsNull() detected. More Information: https://docs.mobilize.net/vbuc/ewis#1049
		...
		public bool CheckDataValidity()
		{
			...
			if (!Information.IsNumeric(txtNatNo.Text))
			{
				MessageBox.Show("Enter Nat Code", "Required", MessageBoxButtons.OK, MessageBoxIcon.Information);
				FirstFocusObject.Focus();
				return false;
			}
		...
		private void Form_Closed(Object eventSender, EventArgs eventArgs)
		{
			...
			try
			{
				...
				frmPayNationality.DefInstance = null;
			}
			catch (Exception exc)
			{
				NotUpgradedHelper.NotifyNotUpgradedElement("Resume in On-Error-Resume-Next Block");

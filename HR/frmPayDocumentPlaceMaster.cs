...
namespace Xtreme
{
	internal partial class frmPayDocumentPlaceMaster
		: System.Windows.Forms.Form
	{

		public frmPayDocumentPlaceMaster()
{
InitializeComponent();
} 
 public  void frmPayDocumentPlaceMaster_old()
			//: base()
		{
		...
		private void frmPayDocumentPlaceMaster_Activated(System.Object eventSender, System.EventArgs eventArgs)
		{
		...
		private void Form_Load()
		{
			...
			try
			{
				...
				txtDocNo.Text = SystemProcedure2.GetNewNumber("Pay_Document_Place_Master", "Place_No");
				//
		...
		public bool saveRecord()
		{
			...
			try
			{
				...
				{
					mysql = "insert into Pay_Document_Place_Master (user_cd, Place_No, L_Place_Name, A_Place_Name , Comments)  ";
				...
				{
					mysql = " update Pay_Document_Place_Master ";
					mysql = mysql + " set  ";
					mysql = mysql + " Place_No = " + txtDocNo.Text;
					mysql = mysql + ", L_Place_Name ='" + txtLDocName.Text + "'";
					mysql = mysql + ", A_Place_Name =N'" + txtADocName.Text + "'";
					mysql = mysql + ", Comments ='" + txtComments.Text + "'";
					mysql = mysql + ", user_cd =" + Conversion.Str(SystemVariables.gLoggedInUserCode);
					mysql = mysql + " where Place_Master_Cd =" + ReflectionHelper.GetPrimitiveValue<string>(mSearchValue);
			...
			catch (System.Exception excep)
			{
				...
				if (mReturnErrorType == 1)
				{
					...
					mSearchValue = ReflectionHelper.GetPrimitiveValue(SystemProcedure2.GetMasterCode("select Place_Master_Cd from Pay_Document_Place_Master where Place_No= " + txtDocNo.Text));
		...
		public object deleteRecord()
		{
			...
			try
			{
				...
				mysql = " select protected from Pay_Document_Place_Master where Place_Master_Cd=" + ReflectionHelper.GetPrimitiveValue<string>(mSearchValue);
				//UPGRADE_WARNING: (1068) GetMasterCode() of type Variant is being forced to Scalar. More Information: https://docs.mobilize.net/vbuc/ewis#1068
				...
				if (!Convert.IsDBNull(mReturnValue))
				{
					...
					try
					{
						mysql = "delete from Pay_Document_Place_Master where Place_Master_Cd=" + Conversion.Str(mSearchValue);
						SqlCommand TempCommand = null;
						TempCommand = SystemVariables.gConn.CreateCommand();
						TempCommand.CommandText = mysql;
						TempCommand.ExecuteNonQuery();
		...
		public void GetRecord(object SearchValue)
		{
			...
			try
			{
				...
				mysql = " select * from Pay_Document_Place_Master where Place_Master_Cd = '" + ReflectionHelper.GetPrimitiveValue<string>(SearchValue) + "'";
				...
				if (rsLocalRec.Read())
				{
					txtDocNo.Text = Convert.ToString(rsLocalRec["Place_No"]);
					txtLDocName.Text = Convert.ToString(rsLocalRec["L_Place_Name"]);
					txtADocName.Text = Convert.ToString(rsLocalRec["A_Place_Name"]) + "";
		...
		public void findRecord()
		{
			...
			object mTempSearchValue = ReflectionHelper.GetPrimitiveValue(SystemProcedure.FindItem(7130000));
			//UPGRADE_WARNING: (1049) Use of Null/IsNull() detected. More Information: https://docs.mobilize.net/vbuc/ewis#1049
		...
		public bool CheckDataValidity()
		{
			...
			{
				MessageBox.Show("Enter the Document Place Code", "Required", MessageBoxButtons.OK, MessageBoxIcon.Information);
				txtDocNo.Focus();
				return false;
			}
			...
			if (txtLDocName.Text == "")
			{
				MessageBox.Show("Enter the Document Place  Name", "Required", MessageBoxButtons.OK, MessageBoxIcon.Information);
				txtLDocName.Focus();
				return false;
			}

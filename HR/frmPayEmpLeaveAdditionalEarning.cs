...
namespace Xtreme
{
	internal partial class frmPayEmpLeaveAdditionalEarning
...
InitializeComponent();
 ...
 public  void frmPayEmpLeaveAdditionalEarning_old()
			//: base()
		{
		...
		private const int grdRemarksColumn = 5;
		...
		private void cmbMastersList_RowChange()
		{
			if (grdVoucherDetails.Col == grdBillingCodeColumn || grdVoucherDetails.Col == grdBillingNameColumn)
			{
				if (grdVoucherDetails.Col == grdBillingCodeColumn)
				{
					...
					grdVoucherDetails.Columns[grdCurrentAmtColumn].Value = 0;
				}
				else
				{
		...
		public bool saveRecord(bool pApprove = false)
		{
			...
			try
			{
				...
				mysql = " delete from pay_employee_leave_additional_earning ";
				...
				for (cnt = 0; cnt <= tempForEndVar; cnt++)
				{
					...
					mysql = " insert into pay_employee_leave_additional_earning ";
					mysql = mysql + " (emp_cd, bill_cd, amount ";
		...
		public void GetRecord(object pSearchValue)
		{
			...
			mysql = mysql + " , pelae.* ";
			mysql = mysql + " from pay_employee_leave_additional_earning pelae ";
			mysql = mysql + " inner join pay_billing_type pbt on pelae.bill_cd = pbt.bill_cd ";
			mysql = mysql + " where pelae.emp_cd = " + Conversion.Str(pSearchValue);

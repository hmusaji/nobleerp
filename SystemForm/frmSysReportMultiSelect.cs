...
InitializeComponent();
 ...
 public  void frmSysReportMultiSelect_old()
			//: base()
		{
		...
		private void Form_Load()
		{
			...
			{
				//UPGRADE_WARNING: (2065) Form property frmSysReportMultiSelect.Controls has a new behavior. More Information: https://docs.mobilize.net/vbuc/ewis#2065
		...
		private void Form_Closed(Object eventSender, EventArgs eventArgs)
		{
			...
			try
			{
				...
				frmSysReportMultiSelect.DefInstance = null;
			}
			catch (Exception exc)
			{
				NotUpgradedHelper.NotifyNotUpgradedElement("Resume in On-Error-Resume-Next Block");
		...
		private void FormatResultGrid()
		{
			...
			grdFindResult.SelectionMode = C1.Win.C1FlexGrid.SelectionModeEnum.Default;
		...
		private void grdFindResult_DoubleClick(Object eventSender, EventArgs eventArgs)
		{
			...
			if (!mResizeInProgress)
			{
				...
				for (Cnt = 0; Cnt <= tempForEndVar; Cnt++)
				{
					...
					{
						filter = Convert.ToString(grdFindResult[grdFindResult.Row, Cnt]);
						break;
					}
				}
				...
				if (mEntryColumnID != 0)
				{
					//Call btnReportOptions_Click(mButtonSeek)
					SystemForms.GetSystemForm(410000, 2, filter);
				}
			}
		...
		private void grdFindResult_KeyDown(Object eventSender, KeyEventArgs eventArgs)
		{
			...
			try
			{
				...
				//    If grdFindResult.Row = grdFindResult.FixedRows Then
				//        grdRecordsCriteria.SetFocus
				//    End If
				//End If
			}
			finally
		...
		private void grdFindResult_KeyUp(Object eventSender, KeyEventArgs eventArgs)
		{
			int KeyCode = (int) eventArgs.KeyCode;
			int Shift = ((int) eventArgs.KeyData) / 65536;
			try
		...
		private void grdFindResult_MouseDown(Object eventSender, MouseEventArgs eventArgs)
		{
			...
			if ((Button == 1) && (Shift == 0))
			{
				if ((mDraggingAllowed) && (!mFGDragInfo.bDragging))
				{
					...
					mFGDragInfo.lSrc = grdFindResult.Row;
		...
		private void grdFindResult_MouseMove(Object eventSender, MouseEventArgs eventArgs)
		{
			int Button = (eventArgs.Button == MouseButtons.Left) ? 1 : ((eventArgs.Button == MouseButtons.Right) ? 2 : 4);
			int Shift = ((int) Control.ModifierKeys) / 65536;
			float x = eventArgs.X * 15;
			...
			if (mFGDragInfo.bCheckDrag && Button == 1)
			{
				if (Math.Abs(x - mFGDragInfo.xDown) + Math.Abs(y - mFGDragInfo.yDown) > DRAGTOL)
				{
					...
					grdFindResult.Cursor = Windows.Cursors.Default;
					grdFindResult.setCellBackColor(Color.Yellow, mFGDragInfo.lSrc, 0);
				}
			}
		...
		private void txtCriteriaBox_KeyDown(Object eventSender, KeyEventArgs eventArgs)
		{
			...
			try
			{
				...
				{ 
					...
					if (txtCriteriaBox.Text == "" || Shift == 2)
					{
						if (grdRecordsCriteria.Col > 0)
						{
							grdRecordsCriteria.Col--;
				...
				else if (KeyCode == ((int) Keys.Right) || KeyCode == ((int) Keys.PageDown))
				{ 
					//**--if creteria box is empty is Ctrl key is pressed
					if (txtCriteriaBox.Text == "" || Shift == 2)
					{
		...
		{
			return mShowAllRecords || ((int) chkCommonSettings[cbShowAllRecords].CheckState) == ((int) TriState.True);
		}
		...
		bool booBusy_grdFindResult_RowColChange = false;
		private void grdFindResult_RowColChange(Object eventSender, EventArgs eventArgs)
		{
			try
			{
				if (Convert.ToString(grdFindResult[grdFindResult.Row, grdFindResult.Col]) == "")
				{
					return;
				}
				...
				int intThisCol = 0;
				int intThisRow = 0;
				if (m_booKeyShift || m_booProcessSelected || booBusy_grdFindResult_RowColChange)
				{
					return;
				}
				booBusy_grdFindResult_RowColChange = true;

				intThisCol = grdFindResult.Col;
				intThisRow = grdFindResult.Row;

				InnovaUpdSupport.PInvoke.SafeNative.user32.LockWindowUpdate(grdFindResult.Handle.ToInt32());

				if (m_booKeyCtrl)
				{
					grdFindResult.Col = 1;
					grdFindResult.Row = intThisRow;
					grdFindResult.ColSel = grdFindResult.Cols.Count - 1;
					grdFindResult.RowSel = intThisRow;
					if (grdFindResult.getCellBackColor() == grdFindResult.Styles.Highlight.BackColor)
					{
						grdFindResult.setCellBackColor(grdFindResult.BackColor);
						grdFindResult.setCellForeColor(grdFindResult.ForeColor);
					}
					else
					{
						grdFindResult.setCellBackColor(grdFindResult.Styles.Highlight.BackColor);
						grdFindResult.setCellForeColor(grdFindResult.Styles.Highlight.ForeColor);
					}
				}
				else
				{
					...
					grdFindResult.Row = 1;
					grdFindResult.ColSel = grdFindResult.Cols.Count - 1;
					grdFindResult.RowSel = grdFindResult.Rows.Count - 1;
					...
					//UPGRADE_ISSUE: (2064) VSFlex8.VSFlexGrid property grdFindResult.FillStyle was not upgraded. More Information: https://docs.mobilize.net/vbuc/ewis#2064
					grdFindResult.setFillStyle(UpgradeStubs.VSFlex8L_FillStyleSettings.getflexFillRepeat());
					grdFindResult.setCellBackColor(grdFindResult.BackColor);
					grdFindResult.setCellForeColor(grdFindResult.ForeColor);
					grdFindResult.Col = 1;
					grdFindResult.Row = intThisRow;
				...
				grdFindResult.Col = intThisCol;
				grdFindResult.Row = intThisRow;

				InnovaUpdSupport.PInvoke.SafeNative.user32.LockWindowUpdate(0);
				booBusy_grdFindResult_RowColChange = false;
			}
			catch
			{
			}
		...
		private void grdFindResult_SelChange(Object eventSender, EventArgs eventArgs)
		{
			try
			{
				...
				intNextCol = grdFindResult.ColSel;
				intNextRow = grdFindResult.RowSel;
				...
				grdFindResult.Row = intThisRow;
				grdFindResult.ColSel = grdFindResult.Cols.Count - 1;
				grdFindResult.RowSel = intNextRow;
				//UPGRADE_ISSUE: (2064) VSFlex8L.FillStyleSettings property FillStyleSettings.flexFillRepeat was not upgraded. More Information: https://docs.mobilize.net/vbuc/ewis#2064
				//UPGRADE_ISSUE: (2064) VSFlex8.VSFlexGrid property grdFindResult.FillStyle was not upgraded. More Information: https://docs.mobilize.net/vbuc/ewis#2064
				grdFindResult.setFillStyle(UpgradeStubs.VSFlex8L_FillStyleSettings.getflexFillRepeat());
				grdFindResult.setCellBackColor(grdFindResult.Styles.Highlight.BackColor);
				...
				grdFindResult.Col = intNextCol;
				grdFindResult.Row = intNextRow;
			}
			catch
			{
			}
		...
		{ //Print routine
			...
			//UPGRADE_WARNING: (1068) GetColumnInformation() of type Variant is being forced to int. More Information: https://docs.mobilize.net/vbuc/ewis#1068
			int mEntryColumnID = ReflectionHelper.GetPrimitiveValue<int>(SystemReports.GetColumnInformation("entry_id_column=1", "column_id", rsDetailsRecordset));
			...
			InnovaUpdSupport.PInvoke.SafeNative.user32.LockWindowUpdate(grdFindResult.Handle.ToInt32());
			lngColSave = grdFindResult.Col;
			lngRowSave = grdFindResult.Row;
			int tempForEndVar = grdFindResult.Rows.Count - 1;
			for (lngRow = grdFindResult.Rows.Fixed; lngRow <= tempForEndVar; lngRow++)
			{
				grdFindResult.Row = lngRow;
				if (grdFindResult.getCellBackColor() == grdFindResult.Styles.Highlight.BackColor || grdFindResult.Styles.EmptyArea.BackColor == grdFindResult.Styles.Highlight.BackColor)
				{
					...
					for (Cnt = 0; Cnt <= tempForEndVar2; Cnt++)
					{
						if (Conversion.Val(ReflectionHelper.GetPrimitiveValue<string>(grdFindResult.Cols[Cnt].UserData).Substring(1)) == mEntryColumnID)
						{
							filter.Append(Convert.ToString(grdFindResult[grdFindResult.Row, Cnt]));
							break;
						}
					}
					j++;
			...
			grdFindResult.Col = lngColSave;
			grdFindResult.Row = lngRowSave;
			InnovaUpdSupport.PInvoke.SafeNative.user32.LockWindowUpdate(0);
		...
		public void KRoutine()
		{
			...
			if (isCheckAll)
			{
				...
				{
					int tempForEndVar2 = grdFindResult.Cols.Count - 1;
					for (ColCnt = 1; ColCnt <= tempForEndVar2; ColCnt++)
					{
						grdFindResult.setCellBackColor(grdFindResult.BackColor, Cnt, ColCnt);
					}
				}
			...
			else
			{
				...
				int tempForEndVar3 = grdFindResult.Rows.Count - 1;
				for (Cnt = 1; Cnt <= tempForEndVar3; Cnt++)
				{
					int tempForEndVar4 = grdFindResult.Cols.Count - 1;
					for (ColCnt = 1; ColCnt <= tempForEndVar4; ColCnt++)
					{
						grdFindResult.setCellBackColor(grdFindResult.Styles.Highlight.BackColor, Cnt, ColCnt);
					}
				}

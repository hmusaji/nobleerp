...
namespace Xtreme
{
	internal partial class frmGLBank
 ...
 public  void frmGLBank_old()
		...
		private void Form_Load()
		{
			...
			try
			{
				...
				//assign a next code
				txtBankNo.Text = SystemProcedure2.GetNewNumber("SM_Bank", "Bank_No");
			}
		...
		public void AddRecord()
		{
			...
			txtBankNo.Text = SystemProcedure2.GetNewNumber("Bank", "Bank_No");
		...
		public bool saveRecord()
		{
			...
			//Dim mParentBankCode As Integer
			//Save a record
			...
			try
			{
				...
				{
					mysql = " INSERT INTO SM_Bank(user_cd, Bank_No, account_no, l_Bank_name, a_Bank_name, comments)";
					mysql = mysql + " VALUES(" + Conversion.Str(SystemVariables.gLoggedInUserCode) + ",";
					mysql = mysql + txtBankNo.Text + ",";
					mysql = mysql + "'" + txtAccountNo.Text + "',";
					mysql = mysql + "'" + txtLBankName.Text + "',";
					mysql = mysql + "N'" + txtABankName.Text + "',";
				...
				{
					mysql = " select time_stamp,protected from SM_Bank where Bank_cd =" + ReflectionHelper.GetPrimitiveValue<string>(mSearchValue);
					...
					mysql = "UPDATE SM_Bank SET ";
					mysql = mysql + " Bank_No =" + txtBankNo.Text + ",";
					mysql = mysql + " account_no ='" + txtAccountNo.Text + "',";
					mysql = mysql + " L_Bank_Name ='" + txtLBankName.Text + "',";
					mysql = mysql + " A_Bank_Name =N'" + txtABankName.Text + "',";
					mysql = mysql + " Comments =N'" + txtComment.Text + "',";
					mysql = mysql + " User_Cd =" + Conversion.Str(SystemVariables.gLoggedInUserCode);
					mysql = mysql + " where Bank_cd=" + Conversion.Str(mSearchValue);
		...
		public bool deleteRecord()
		{
			...
			bool result = false;
			string mysql = " select protected from SM_Bank where Bank_cd=" + ReflectionHelper.GetPrimitiveValue<string>(mSearchValue);
			//UPGRADE_WARNING: (1068) GetMasterCode() of type Variant is being forced to Scalar. More Information: https://docs.mobilize.net/vbuc/ewis#1068
			...
			if (!Convert.IsDBNull(mReturnValue))
			{
				...
				try
				{
					mysql = "delete from SM_Bank where Bank_cd=" + Conversion.Str(mSearchValue);
					SqlCommand TempCommand = null;
					TempCommand = SystemVariables.gConn.CreateCommand();
					TempCommand.CommandText = mysql;
					TempCommand.ExecuteNonQuery();
		...
		public void GetRecord(object SearchValue)
		{
			...
			try
			{
				...
				mysql = " select * from SM_Bank where Bank_cd= " + ReflectionHelper.GetPrimitiveValue<string>(SearchValue);
				...
				if (localRec.Read())
				{
					...
					txtBankNo.Text = Convert.ToString(localRec["Bank_No"]);
					txtAccountNo.Text = Convert.ToString(localRec["account_no"]) + "";
					txtLBankName.Text = Convert.ToString(localRec["l_Bank_Name"]);
					txtABankName.Text = Convert.ToString(localRec["a_Bank_Name"]) + "";
		...
		public void findRecord()
		{
			...
			object mTempSearchValue = ReflectionHelper.GetPrimitiveValue(SystemProcedure.FindItem(7030000));
			//UPGRADE_WARNING: (1049) Use of Null/IsNull() detected. More Information: https://docs.mobilize.net/vbuc/ewis#1049
		...
		public bool CheckDataValidity()
		{
			...
			if (!Information.IsNumeric(txtBankNo.Text))
			{
				MessageBox.Show("Enter Bank Code", "Required", MessageBoxButtons.OK, MessageBoxIcon.Information);
				FirstFocusObject.Focus();
				return false;
			}
		...
		private void txtBankNo_DropButtonClick(Object Sender, EventArgs e)
		{
		...
		public void FindRoutine(string pObjectName)
		{
			...
			//            txtParentBankNo.Text = mTempSearchValue(1)
			//            txtParentBankName.Text = IIf(gPreferedLanguage = english, mTempSearchValue(2), mTempSearchValue(3))
			//            Call txtParentBankNo_LostFocus
			//        End If
			//    Case Else
			//        Exit Sub
			//End Select
